...
void limits_init()
{
    ...
    // Set as input pins
    LIMIT_DDR(0) &= ~(1<<LIMIT_BIT(0));
    LIMIT_DDR(1) &= ~(1<<LIMIT_BIT(1));
    LIMIT_DDR(2) &= ~(1<<LIMIT_BIT(2));
    ...
    #else
      LIMIT_PORT(0) |= (1<<LIMIT_BIT(0));  // Enable internal pull-up resistors. Normal high operation.
      LIMIT_PORT(1) |= (1<<LIMIT_BIT(1));  // Enable internal pull-up resistors. Normal high operation.
      LIMIT_PORT(2) |= (1<<LIMIT_BIT(2));  // Enable internal pull-up resistors. Normal high operation.
    #endif
    #ifndef DISABLE_HW_LIMITS
      if (bit_istrue(settings.flags,BITFLAG_HARD_LIMIT_ENABLE)) {
        HLIMIT_PCMSK |= HLIMIT_MASK; // Enable specific pins of the Pin Change Interrupt
        PCICR |= (1 << HLIMIT_INT); // Enable Pin Change Interrupt
      } else {
        limits_disable();
      }
...
void limits_disable()
{
  #ifdef DEFAULTS_RAMPS_BOARD
    #ifndef DISABLE_HW_LIMITS
     HLIMIT_PCMSK &= ~HLIMIT_MASK;  // Disable specific pins of the Pin Change Interrupt
     PCICR &= ~(1 << HLIMIT_INT);  // Disable Pin Change Interrupt
    #endif
  #else
...
#ifdef DEFAULTS_RAMPS_BOARD  
  static volatile uint8_t * const limit_pins[N_AXIS] = {&LIMIT_PIN(0), &LIMIT_PIN(1), &LIMIT_PIN(2)};
  static const uint8_t limit_bits[N_AXIS] = {LIMIT_BIT(0), LIMIT_BIT(1), LIMIT_BIT(2)};
#endif // DEFAULTS_RAMPS_BOARD
...
uint8_t limits_get_state()
{
  uint8_t limit_state = 0;
    ...
    for (idx=0; idx<N_AXIS; idx++) {
      pin = *limit_pins[idx] & (1<<limit_bits[idx]);
      pin = !!pin;
...
// your e-stop switch to the Arduino reset pin, since it is the most correct way to do this.
    ...
    ISR(HLIMIT_INT_vect) // DEFAULT: Limit pin change interrupt process. 
    {
